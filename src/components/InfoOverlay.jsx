  ...
  return (
    <div onClick={()=>setShowInfo(false)} style={{
      ...
      <div className="pop" onClick={e=>e.stopPropagation()} style={{
        ...
        background:"#1a1a1f",border:`1px solid ${col}30`,borderRadius:R,
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {info.lines.map((line,i)=>(
            <div key={i} style={{
              ...
              letterSpacing:1,paddingLeft:14,borderLeft:`2px solid ${col}40`,
            }}>{line}</div>
          ))}
        </div>
        <button onClick={()=>setShowInfo(false)} style={{
          width:"100%",marginTop:24,padding:"14px 0",
          background:`${col}15`,border:`1px solid ${col}30`,borderRadius:R,
          fontFamily:BB,fontSize:14,letterSpacing:5,color:col,cursor:"pointer",
        }}>GOT IT</button>
      </div>
    </div>
  );
