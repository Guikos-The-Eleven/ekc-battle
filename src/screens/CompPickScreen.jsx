...
export default function CompPickScreen({ mode, expandedComp, setExpandedComp, setSelectedComp, setSelectedDiv, setOpenList, setScreen }) {
  ...
  const upcoming = COMPS_SORTED.filter(c => !isCompPast(c));
  const past     = COMPS_SORTED.filter(c =>  isCompPast(c)).reverse();
    ...
    <div style={root}>
      ...
      <div style={page}>
        ...
        <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",margin:"0 -24px",padding:"0 24px"}}>
          {(() => {
            const renderComp = (comp, {past=false}={}) => {
              const isOpen = expandedComp===comp.key;
              const isSoon = comp.soon;
              const dimmed = isSoon || past;
              const baseOpacity = isSoon ? 0.4 : past ? 0.55 : 1;
              return (
                <div key={comp.key} style={{marginBottom:8,opacity:isOpen?1:baseOpacity,transition:"opacity 0.15s"}}>
                  <button className="tap" onClick={()=>!isSoon&&setExpandedComp(isOpen?null:comp.key)} style={{
                    ...
                    filter: past && !isOpen ? "grayscale(0.4)" : "none",
                  }}>
                    <div style={{textAlign:"left"}}>
                      <div style={{display:"flex",alignItems:"center",gap:10}}>
                        ...
                        {past && !isSoon && <span style={{fontFamily:BC,fontSize:11,letterSpacing:2,color:C.muted,fontWeight:600,
                          border:`1px solid ${C.border}`,padding:"2px 0",borderRadius:R,minWidth:52,textAlign:"center",display:"inline-block"}}>PAST</span>}
                      </div>
